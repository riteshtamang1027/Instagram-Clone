...
export default function Profile_Feed_Section() {
  return (
    <div className="">
      <div className="flex gap-8 sm:ml-60">
      ...
      <div className="grid grid-cols-3 gap-x-4 p-2 ">
        {Posts.map((item, index) => (
          <div
            key={index}
            className="  w-[20vw] h-[40vh] relative cursor-pointer overflow-hidden py-2  "
          >
            <img height={200} width={300} src={item.img} alt="Image" />

            <div className=" w-[20vw] h-[40vh] top-0 absolute flex items-center justify-center gap-12 opacity-0 hover:opacity-100 text-white  ">
              <Heart />
              <MessageCircle />
            </div>
          </div>
